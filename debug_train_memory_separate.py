...
def train(segmentation_module, iterator, optimizers, history, epoch, cfg):
    ...
    for i in range(100):
        ...
        # forward pass
        #print(batch_data)
        loss, acc, qread, qval = segmentation_module(batch_data)
        loss = loss.mean()
        acc = acc.mean()
        ...
        tic = time.time()
    np.save('debug/qread.npy', np.stack(qread_all))
    np.save('debug/qval.npy', np.stack(qval_all))
...
def main(cfg, gpus):
    ...
    if cfg.MODEL.arch_decoder.endswith('deepsup'):
        segmentation_module = SegmentationAttentionSeparateModule(
            net_enc_query, net_enc_memory, net_att_query, net_att_memory, net_decoder, crit, cfg.TRAIN.deep_sup_scale, zero_memory=cfg.MODEL.zero_memory, random_memory_bias=cfg.MODEL.random_memory_bias, random_memory_nobias=cfg.MODEL.random_memory_nobias, random_scale=cfg.MODEL.random_scale, debug=True)
    else:
        segmentation_module = SegmentationAttentionSeparateModule(
            net_enc_query, net_enc_memory, net_att_query, net_att_memory, net_decoder, crit, zero_memory=cfg.MODEL.zero_memory, random_memory_bias=cfg.MODEL.random_memory_bias, random_memory_nobias=cfg.MODEL.random_memory_nobias, random_scale=cfg.MODEL.random_scale, debug=True)
